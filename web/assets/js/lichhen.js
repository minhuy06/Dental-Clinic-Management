...
/**
 * ==================== NHA KHOA 5AE - QUẢN LÝ LỊCH HẸN ====================
 * File: appointment.js
 * Chức năng: Xử lý toàn bộ logic cho trang quản lý lịch hẹn
 */
...
let appointments = [
    { id: 1, time: "08:00", patientName: "Nguyễn Văn An", patientPhone: "0901000001", doctor: "BS. Nguyễn Hải", services: [{ id: 1, name: "Khám tổng quát", price: 100000, quantity: 1 }], totalPrice: 100000, room: "Phòng 1", date: "2026-04-22", status: "confirmed" },
    { id: 2, time: "09:00", patientName: "Trần Thị Bích", patientPhone: "0901000002", doctor: "BS. Trần Tâm", services: [{ id: 15, name: "Niềng răng mắc cài kim loại", price: 25000000, quantity: 1 }], totalPrice: 25000000, room: "Phòng 2", date: "2026-04-22", status: "confirmed" },
    { id: 3, time: "10:00", patientName: "Lê Hoàng Nam", patientPhone: "0901000003", doctor: "CSKH. Mai Anh", services: [{ id: 5, name: "Nhổ răng khôn", price: 1000000, quantity: 2 }], totalPrice: 2000000, room: "Phòng 3", date: "2026-04-22", status: "pending" },
    { id: 4, time: "14:00", patientName: "Phạm Thu Hà", patientPhone: "0901000004", doctor: "BS. Phạm Hương", services: [{ id: 7, name: "Tẩy trắng răng Laser", price: 2500000, quantity: 1 }], totalPrice: 2500000, room: "Phòng 1", date: "2026-04-22", status: "confirmed" },
    { id: 5, time: "15:30", patientName: "Hoàng Vĩnh Khang", patientPhone: "0901000005", doctor: "BS. Hoàng Quân", services: [{ id: 14, name: "Cấy ghép Implant", price: 15000000, quantity: 1 }], totalPrice: 15000000, room: "Phòng 4", date: "2026-04-22", status: "completed" },
    { id: 6, time: "08:30", patientName: "Đặng Phương Hoa", patientPhone: "0901000006", doctor: "CSKH. Mai Anh", services: [{ id: 4, name: "Nhổ răng sữa", price: 100000, quantity: 3 }, { id: 1, name: "Khám tổng quát", price: 100000, quantity: 1 }], totalPrice: 400000, room: "Phòng 1", date: "2026-04-23", status: "pending" }
];
...
function getStatusIcon(status) {
    let map = { 'confirmed': 'fa-check-circle', 'pending': 'fa-hourglass-half', 'cancelled': 'fa-times-circle', 'completed': 'fa-check-double' };
...
function resetForm() {
    ...
    document.getElementById('appointmentDate').value = formatDate(currentDate);
    selectedServices = [];
    if (document.querySelectorAll('.service-checkbox input')) {
...
function updateRoom() {
    ...
    let roomMap = { 
        'BS. Nguyễn Hải': 'Phòng 1', 
        'BS. Trần Tâm': 'Phòng 2', 
        'BS. Lê Quang': 'Phòng 3', 
        'BS. Phạm Hương': 'Phòng 1', 
        'BS. Hoàng Quân': 'Phòng 4',
        'CSKH. Mai Anh': 'Phòng 1'
    };
    document.getElementById('room').value = roomMap[doctor] || 'Phòng 1';
}
...
function renderAppointments() {
    ...
    tbody.innerHTML = filtered.map(apt => `
        <tr>
            ...
            <td><div class="doctor-tag"><i class="fas fa-headset"></i> ${apt.doctor}</div></div>
            <td>
            ...
            <td><i class="fas fa-door-open"></i> ${apt.room}</div>
            <td>
                <div class="status-dropdown">
                    <div class="status-badge ${getStatusClass(apt.status)}" onclick="toggleStatusMenu(${apt.id})">
                        <i class="fas ${getStatusIcon(apt.status)}"></i> ${getStatusText(apt.status)}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('addAppointmentBtn').onclick = openAddModal;
    
    document.querySelectorAll('.view-options button').forEach(btn => {
        btn.onclick = () => changeView(btn.getAttribute('data-view'));
    });
    
    document.getElementById('filterDoctor').onchange = filterAppointments;
    document.getElementById('searchInput').onkeyup = filterAppointments;
    
    document.querySelector('.close').onclick = closeModal;
    document.querySelector('.btn-cancel').onclick = closeModal;
    document.querySelector('.btn-save').onclick = saveAppointment;
    
    window.onclick = (e) => { if (e.target === document.getElementById('appointmentModal')) closeModal(); };
});