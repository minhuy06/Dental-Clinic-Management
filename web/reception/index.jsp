...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/reception-dashboard.css?v=20260518b">
...
<c:url var="urlReceptionPeriodYear" value="/reception-dashboard"><c:param name="view" value="year"/><c:param name="ngay" value="${selectedDateNgay}"/><c:param name="loc" value="${rcvLoc}"/></c:url>
    <!-- ==================== HEADER MÀU XANH ==================== -->
    <div class="header">
        <jsp:include page="../components/clinic-logo.jsp" />
        <ul class="nav-menu">
    ...
    <script id="receptionContextJson" type="application/json">{"baseUrl":"${pageContext.request.contextPath}/reception-dashboard","selectedDate":"<c:out value="${selectedDateNgay}"/>","viewMode":"<c:out value="${rcvView}"/>","loc":"<c:out value="${rcvLoc}"/>","histTotal":<c:choose><c:when test="${receptionHistBanner eq true}">true</c:when><c:otherwise>false</c:otherwise></c:choose>}</script>
    <script id="receptionServicesJson" type="application/json"><c:out value="${empty serviceListJson ? '[]' : serviceListJson}" escapeXml="false"/></script>
    <script id="receptionRoomsJson" type="application/json"><c:out value="${empty roomListJson ? '[]' : roomListJson}" escapeXml="false"/></script>
